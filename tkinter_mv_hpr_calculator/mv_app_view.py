...
class MVAppView:
    ...
    def setup_ui(self, on_calculate):
        self.root.title("Market Value and Return Calculator")
        ...
        tk.Label(self.root, text="Start Date:").grid(row=1, column=0)
        self.start_date_entry = tk.Entry(self.root)
        self.start_date_entry.grid(row=1, column=1)

        tk.Label(self.root, text="End Date:").grid(row=2, column=0)
        self.end_date_entry = tk.Entry(self.root)
        self.end_date_entry.grid(row=2, column=1)

        tk.Label(self.root, text="Shares Purchased:").grid(row=3, column=0)
        self.shares_entry = tk.Entry(self.root)
        self.shares_entry.grid(row=3, column=1)

        self.calc_button = tk.Button(self.root, text="Calculate", command=on_calculate)
        self.calc_button.grid(row=4, column=1)

        self.results_text = tk.Text(self.root, height=4, width=50)
